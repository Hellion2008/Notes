...
def main():
  ...
  while(key_input != 0):
      if key_input == 1:
        ...
        edit_note(int(input(Fore.YELLOW + 'Введите номер заметки: ')))
      elif key_input == 4:
...
def create_note_json():
  id = getId()
  title = input(Fore.YELLOW + 'Введите заголовок: ' + Fore.RESET)
  text = input(Fore.YELLOW + 'Текст заметки: ' + Fore.RESET)
  time_create = datetime.datetime.now().strftime("%d-%b-%Y (%H:%M:%S)")
  path_file = str(Path('notes', f'{id}.json'))
  save_to_json(path_file, id, title, text, time_create, '-')
...
def edit_note(id):
    ...
    title = input(Fore.YELLOW + 'Новый заголовок: ' + Fore.RESET)
    text = input(Fore.YELLOW + 'Новый текст заметки: ' + Fore.RESET)
    time_change = datetime.datetime.now().strftime("%d-%b-%Y (%H:%M:%S)")
    save_to_json(path_file, note_dic['id'], title, text, note_dic['time_create'], time_change)
...
def save_to_json(path_file, file_name, file_title, file_text, time_create, time_change):
  ...
  new_file.write(f"\t\"time_change\": \"{time_change}\"\n")
  new_file.write("}\n")
  new_file.close()
